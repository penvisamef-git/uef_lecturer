...
function AnalyticsTab({ classData, genderStats, gradeStats, highAbsentStudents, subjectScores }) {
  // Calculate total students from classData
  const totalStudents = classData?.students?.length || 0;
  
  // Calculate active students (all students are active by default in new model)
  const activeStudents = classData?.students?.filter(s => s.student_status === "active" || !s.student_status)?.length || 0;
  
  // Calculate inactive students (dropped or inactive)
  const inactiveStudents = classData?.students?.filter(s => s.student_status === "dropped" || s.student_status === "inactive")?.length || 0;
            ...
            {highAbsentStudents.length > 0 ? (
              <div className="list-group">
                {highAbsentStudents.map((student, index) => {
                  // Calculate total absence from attendance array
                  const totalAbsence = student.attendance?.reduce((sum, a) => {
                    return sum + (a.total_absence_unreport || 0) + (a.total_absence_report || 0);
                  }, 0) || 0;
                  
                  return (
                    <div key={index} className="list-group-item d-flex justify-content-between align-items-center">
                      <span className="siemreap-regular">{student.student_id?.firstname || ""} {student.student_id?.lastname || ""}</span>
                      <span className="badge" style={{ background: "#dc3545", color: "white" }}>{totalAbsence} ដង</span>
                    </div>
                  );
                })}
              </div>
